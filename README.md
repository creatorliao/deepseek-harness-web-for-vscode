...
- **目录树拖拽即 `@` 引用**：从资源管理器把文件（可多选、可含文件夹）拖到 DSH 输入框，就会写入 `@工作区相对路径`——与用 `@` 补全选中该文件**功能等价**（模型据此自己读取文件）。
  - ⚠️ **必须按住 `Shift` 再拖**：VS Code 在窗口内拖拽期间会关掉 webview 的指针事件，只有按住 `Shift` 才放行（这是 VS Code 官方对 "文件拖进 webview" 的解法）。不按 `Shift` 时文件会被 VS Code 在编辑器区打开。
  - **更省事的两个入口（不需要 Shift）**：
    - **快捷键**：光标在编辑器里时按 `Ctrl+Alt+A`（macOS `Cmd+Alt+A`）→ 当前文件变成 `@` 引用；键位可在「键盘快捷方式」里自行改。
    - **右键**：资源管理器里选中文件（可多选）→ 右键 → **添加到 DSH 输入框（@ 引用）**。
  - ℹ️ 写入的是**纯文本 `@路径`**：上游 `@` 补全产生的 chip 序列化后就是这串文本，发给模型的内容逐字节相同——所以视觉上没有 chip，但语义完全一致。
...
`0.1.5-rc.2 → 0.1.6-alpha.2` 之间官方有若干**需要你动手**的变更（完整逐字发布说明见 [`docs/03-Resources/20260918-01`](docs/03-Resources/20260918-01-官方DSH发布说明_0.1.5-rc.2到0.1.6-alpha.2.md)）：

| 变更 | 你需要做什么 |
|---|---|
| DeepSeek 默认改用 Messages 协议，**自定义 API 根地址**保留不变 | 若你手动配过**旧官方根地址**，删掉该配置或改为 `https://api.deepseek.com/anthropic` |
| 默认模型列表**移除 V4 Flash 与 V4 Flash Vision Exp** | 依赖这两个模型的话，需在设置里自行加回 |
| 内置 **E2B 执行后端被移除** | 用过它的自定义配置要改 |
| PTC 包名/服务名统一为 `ptc-runtime` 系列、工作流执行器改名 `workflow-ptc` | 有自定义插件/配置的话要跟着改名 |
| **默认不再启用 Ralph** | 需要时手动开启 |
| 实验性 Team 模式统一用 `spawn_teammate`（关闭 `subagent`/`subagent_fork`） | 用到 Team 模式的话改调用名 |
| **Web 用户终端使用系统用户权限，不受 Agent 沙箱限制** | 安全语义变化：面板里的终端不再受沙箱约束，知情后再决定是否开 |
| 配置热更新**取消事务回滚**、请求图片缓存换到 `DSH_HOME/cache/attachments/request-images`、Node PTC 改独立进程（`process.env` 为空） | 排障时注意这三处行为与旧版不同 |

> 磁盘占用提醒：该版本 `node_modules` 从 222.9 MiB 涨到 **560.6 MiB**（+325.1 MiB，来自 Office 预览的 LibreOffice 组件）；启动则快约 3 倍（10.3–11.3 s → 3.5–3.9 s）。数据为 2026-09-18 实测。
