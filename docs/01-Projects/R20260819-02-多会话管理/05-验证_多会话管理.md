...
| 会话按钮用 `.icon-btn` 类 | **全内联 20×20**（Gemini 分析：全局 `button {width:100%}` 污染） | 用户 F5：第二个按钮被全局 button 规则撑爆挤出可视区（docs/01-Projects/R20260819-02-多会话管理/09-修复_会话按钮横向偏移.md） |
...
*关联文档：req.md ｜ solution.md ｜ plan.md ｜ review-by-gemini.md ｜ discussion.md ｜ docs/01-Projects/R20260819-02-多会话管理/09-修复_会话按钮横向偏移.md*
