...
**日期**: 2026-08-17
**来源**: [discussion.md](01-讨论_桥架构与MVP立项.md)、[架构提案](09-架构提案_by-deepseek.md)、[市场分析](../../03-Resources/20260817-01-市场与竞品数据快照.md)
...
### R2 — 内嵌 Web UI（2026-08-17 修订：侧边栏 → 编辑器标签页）
- 在**编辑器标签页**（WebviewPanel，展示方式对齐 Claude Code）加载 DSH Web UI，功能完整可用（会话、工作区、设置、插件、Goal、Workflow）。
- **修订原因**（用户 2026-08-17）：侧边栏视图会与资源管理器等侧栏树视图重叠，编辑器标签页与文件编辑并存、互不遮挡。
- **验收**：
  - 执行启动后打开**编辑器标签页**并渲染 DSH 界面，无白屏/无控制台致命错误；
  - 标签页与文件编辑器并存；打开资源管理器树不遮挡 DSH 界面；
