...
| 决策 | 理由 | 备选与否决原因 |
|---|---|---|
| 写入**纯文本 `@path`**，不复刻 chip | 上游 chip 的 `serialize()` 就是这串文本（`05-事实` §落文本 vs 结构化节点），复刻无功能收益；上游也未开放程序化插入 chip 的入口 | 客户端插件 `slash/input-insert-reference`：要装进 `$DSH_HOME`，上游一升级就坏 |
...
⚠️ 同族项目的验收清单同样是空的（其"宿主能否收到 drop""insertText 能否落进输入框"两跳未实测）——所以本轮**不把它的存在当作可行性证据**，而是自己重新取证（`04-事实` / `05-事实`）。

## 5. 遗留

见 [10-待办](10-待办_目录树拖拽到Composer.md)：核心是 **T11 真机鼠标验收**（本机无 GUI 自动化），以及"若入口 A 真机不通过则启用 E3（TreeView 投放区）"的预案。

---

*关联文档：[08-验证](08-验证_目录树拖拽到Composer.md) ｜ [03-分析](03-分析_参考项目与可行性.md)*
