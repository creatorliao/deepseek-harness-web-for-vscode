...
**日期**: 2026-09-14 ｜ **状态**: 生效（当前**唯一**在售渠道）｜ **定位**: Open VSX 的账号准备、Token、namespace 与手工发布底层命令
**适用项目**: `creatorliao/deepseek-harness-for-vscode` ｜ **扩展 ID**: `creatorliao.deepseek-harness-for-vscode`

> 渠道现状与完整发版流程见 [发布与版本规范](20260914-07-发布与版本规范.md)。本文件是 Open VSX 这一渠道的**操作细节**。
...
在本地终端执行以下命令创建 Namespace（`creatorliao`）：
```sh
npx --yes ovsx create-namespace creatorliao -p <YOUR_OVSX_TOKEN>
```
*注：也可以使用已封装的 `make namespace`（需先导出 `OVSX_TOKEN` 环境变量）。*
*现状（2026-09-14 实查）：`creatorliao` 这个 namespace 在 Open VSX 上尚无任何扩展；历史旧的 id 是 `floatinghotpot.deepseek-harness-for-vscode`（属于原作者，与本项目无关）。*
...
### 2.2 手动底层命令（排查用）
```sh
# 1. 编译并打包到 dist/（只保留最新一份，文件名带版本号）
npm run package

# 2. 发布至 Open VSX（上传 dist/ 下的 vsix）
npx --yes ovsx publish dist/creatorliao.deepseek-harness-for-vscode-<version>.vsix -p "$OVSX_TOKEN"
```
...
- **根因**: 未执行 CLI 创建命名空间操作。
- **对策**: 运行 `npx --yes ovsx create-namespace creatorliao -p <TOKEN>` 完成一次性创建。
