...
- **快捷键入口：`Ctrl+Alt+A`（macOS `Cmd+Alt+A`）把当前文件加入 DSH 输入框**（光标在编辑器里时生效）。与右键入口走同一条翻译层与写入层；键位在 VS Code 1.105.1 的默认键位表里**零冲突**（实测），也可在「键盘快捷方式」里自行改键。
  - 为什么加这一条：真机实测"拖到 composer"被 VS Code 平台挡住（webview 拖拽期间 iframe 被置 `pointer-events:none`，只有 Shift 放行，且 Shift 必须在指针进入面板**之前**按住）。**与其硬扛拖拽，不如把入口做成一次按键**——这也是 Cursor 的主用法（`Ctrl+L`），拖拽只是它的次要入口。
  - 没有打开文件时给出中文提示（指引用右键入口），不静默失败。
  - 资源管理器的**多选**仍然走右键入口：VS Code 没有把"资源管理器选中项"暴露给扩展的官方 API，键位路径只能取"当前编辑的文件"。

### 说明

- 拖拽入口保持不变（Shift 方案仍在，待真机复测）；后备方案「TreeView 投放条」（官方 `TreeDragAndDropController`，宿主直接拿到拖入的 URI 列表、不受 webview 屏蔽影响）已评估并登记，**是否新增这块常驻 UI 待用户决定** → 主题夹 `07-待办` E-C。

## [0.5.2] - 2026-09-17

### 修复

- **右键「添加到 DSH 输入框（@ 引用）」写进去了却提示失败，照提示粘贴就变成两条相同引用**（用户实测反馈）：
  - **根因**：写入层用**同步 DOM 读**判定"写没写进去"，而 DSH 的 composer 是 Lexical——编辑先进编辑器状态、DOM 在**之后的 tick** 才提交。于是文本明明已经写进去（真机探针实测：1 次），代码却回报 `{ok:false, reason:"rejected"}`，宿主据此**把引用复制到剪贴板并弹提示"请按 Ctrl+V 粘贴"**——用户一粘贴就成了两条。同一判据还会无谓触发 `execCommand` 兜底，**两次提交在较慢的环境里都落地同样是两条**。
  - **修复**：写入层改为**等待 + 按内容判定**（判据是"引用确实在输入框里"，而不是"DOM 字符串变没变"）；单步预算 paste 600ms / execCommand 400ms，只有前一步确实没落地才走兜底；成功即回报 `ok:true`，宿主不再写剪贴板、不再提示粘贴。
  - 顺带修掉同一判据的另一面：**粘贴若是"替换一段内容完全相同的选区"，DOM 字符串根本不变而写入其实成功了**——用"变了"判定照样误报失败、照样骗用户去粘贴。
- **新增真机探针 `npm run probe:composer`**（`scripts/probe-composer-insert.js`）：起真实 `dsh web` + headless Chromium、**注入真 `bridge-client.js`**，实测"写入几次 / 回报什么 / 拖拽负载有没有转发"。这个缺陷原本逃过了全部单测——因为旧的假 composer 在 `dispatchEvent` 里**同步**改 DOM，把"异步提交"这一关键性质抹平了，永远走不到失败分支。探针把它补成了机器可查的一步，并发版清单要求"改写入面必跑"。
- **拖拽入口**：真机实测**不生效**（尚未确认当时是否按住 Shift）。页面侧链路已被探针证明正常（VS Code 形状的 `dragover`/`drop` 被接管、负载原样转发），因此失败点在 VS Code 是否把事件交给 iframe——即 webview 拖拽的 Shift 前提。复测步骤与后备方案（TreeView 投放区）见主题夹 `07-待办` 的 T13/E3。

### 文档

- [扩展与 Webview 规范](docs/02-Areas/20260914-02-扩展与Webview规范.md) 新增 **§3.1「往上游富文本编辑器写入：不得用同步 DOM 读判定成败」**（含两次真实误判的成因），并加入该规范的提交前自检清单。
